	...
	<body bgcolor="#D3D3D3">
		<div class="container1">
			...
			<?php				
				...
				$sql = "SELECT * FROM men WHERE tip='camasa'";
				$query = mysqli_query($db,$sql);
				while($row = mysqli_fetch_assoc($query)){
					echo '<a class="linkimgcont1" href="buy.php?id='.$row['id'].'&tip=1" title="Cămăși">';
			...
			<?php				
				...
				$parolabazadedate = "";
				$db = mysqli_connect($localhost , $idbazadedate , $parolabazadedate , 'shop');
				$sql = "SELECT * FROM men WHERE tip='pantaloni'";
				$query = mysqli_query($db,$sql);
				while($row = mysqli_fetch_assoc($query)){
					echo '<a class="linkimgcont1" href="buy.php?id='.$row['id'].'&tip=1" title="Pantaloni scurți">';
					echo '<img class="imgcont1" src="data:image/jpeg;base64,'.base64_encode($row['image']).'"></img></a>';
				}
			?>
			<ul class="container1_1">
				<p class="textinfocont1" title="Tricouri">Tricouri</p>
			</ul>
			<?php				
				$localhost = "localhost";
				$idbazadedate = "root";
				$parolabazadedate = "";
				$db = mysqli_connect($localhost , $idbazadedate , $parolabazadedate , 'shop');
				$sql = "SELECT * FROM men WHERE tip='bluza'";
				$query = mysqli_query($db,$sql);
				while($row = mysqli_fetch_assoc($query)){
					echo '<a class="linkimgcont1" href="buy.php?id='.$row['id'].'&tip=1" title="Tricouri">';
					echo '<img class="imgcont1" src="data:image/jpeg;base64,'.base64_encode($row['image']).'"></img></a>';
				}
			?>
			...
			<?php				
				...
				$parolabazadedate = "";
				$db = mysqli_connect($localhost , $idbazadedate , $parolabazadedate , 'shop');
				$sql = "SELECT * FROM men WHERE tip='incaltaminte'";
				$query = mysqli_query($db,$sql);
				while($row = mysqli_fetch_assoc($query)){
					echo '<a class="linkimgcont1" href="buy.php?id='.$row['id'].'&tip=1" title="Încălțăminte">';
					echo '<img class="imgcont1" src="data:image/jpeg;base64,'.base64_encode($row['image']).'"></img></a>';
				}
			?>
			...
			<?php				
				...
				$parolabazadedate = "";
				$db = mysqli_connect($localhost , $idbazadedate , $parolabazadedate , 'shop');
				$sql = "SELECT * FROM men WHERE tip='jeans'";
				$query = mysqli_query($db,$sql);
				while($row = mysqli_fetch_assoc($query)){
					echo '<a class="linkimgcont1" href="buy.php?id='.$row['id'].'&tip=1" title="Jeans">';
					echo '<img class="imgcont1" src="data:image/jpeg;base64,'.base64_encode($row['image']).'"></img></a>';
				}
			?>
		</div>
		<div style="width:auto;height:auto;">
			<ul>
				...
				<?php
					echo '<li class="right login"><a class="home" href="login.php" title="Login"><p class="login">Login</p></a></li>';
